import { bumpGlobalCounter, getSharedGlobalVariable, getToolboxIframe, getUniqueKey, onConstructorCall, overrideGlobalFunction, overrideInstanceMethod, overrideStaticMethod, setSharedGlobalVariable, updateSharedGlobalVariable } from '../utils/internal';
...
import { IS_AUDIO_PLAYBACK_SUPPORTED, SOUND_PLAYBACK_STRATEGY_AUDIO, SOUND_PLAYBACK_STRATEGY_HOWLER, SOUND_SPEED_NORMAL, SOUND_SPEED_SLOW, SOUND_TYPE_EFFECT, SOUND_TYPE_UNKNOWN, SOUND_TYPE_TTS_SENTENCE, SOUND_TYPE_TTS_WORD } from './sounds';
/**
 * @type {string}
 */
...
export const onUserDataLoaded = _arg10 => {
  return registerXhrRequestEventListener(EVENT_TYPE_USER_DATA_LOADED, _arg10);
};
/**
 * @type {Function}
...
export const onPracticeSessionLoaded = _arg11 => {
  return registerXhrRequestEventListener(EVENT_TYPE_PRACTICE_SESSION_LOADED, _arg11);
};
/**
 * @type {Function}
...
export const onStoryLoaded = _arg12 => {
...
export const onForumDiscussionLoaded = _arg13 => {
...
export const onDictionaryLexemeLoaded = _arg14 => {
...
export const onUserCoursesLoaded = _arg15 => {
  return registerDerivedEventListener(EVENT_TYPE_USER_COURSES_LOADED, EVENT_TYPE_USER_DATA_LOADED, _arg15, userData => {
    let payload;

    if (isObject(userData) && isArray(userData.courses) && isString(userData.fromLanguage) && isString(userData.learningLanguage)) {
      const courses = userData.courses.map(parseCourse).filter(isObject);
      const currentCourse = courses.find(_it2 => {
        return _it2.fromLanguage === userData.fromLanguage && _it2.toLanguage === userData.learningLanguage;
      });
      payload = [{
        courses,
        currentCourse
...
export const onPracticeChallengesLoaded = _arg16 => {
...
const getSpeakerSoundData = (sound, type, language) => ({
  url: sound.url,
  type,
  speed: (sound === null || sound === void 0 ? void 0 : sound.speed.value) || SOUND_SPEED_NORMAL,
  language
});
/**
  ...
  if (IS_AUDIO_PLAYBACK_SUPPORTED) {
    onConstructorCall('Audio', sound => {
      if ('' !== sound.src.trim() && !getSharedGlobalVariable(KEY_IS_HOWLER_USED)) {
        dispatchEvent(EVENT_TYPE_SOUND_INITIALIZED, getSoundEventPayload(sound, sound.src, SOUND_PLAYBACK_STRATEGY_AUDIO));
      }
    });
  }
...
const registerSoundPlaybackEventListener = (event, callback) => {
  ...
  if (IS_AUDIO_PLAYBACK_SUPPORTED) {
    ...
    overrideStaticMethod('URL', 'createObjectUrl', originalCreateObjectUrl => function (object) {
      ...
      return isBlob(object) && audioBlobUrlMap !== null && audioBlobUrlMap !== void 0 && audioBlobUrlMap.has(object) ? audioBlobUrlMap.get(object) : originalCreateObjectUrl.call(this, object);
    });
      ...
      return originalFetch.call(this, resource, init).then(response => {
        ...
        return response.blob().then(blob => {
          if (blob.type.indexOf('audio') === 0) {
            ...
            setSharedGlobalVariable(KEY_AUDIO_BLOB_SOUND_URL_MAP, audioBlobSoundUrlMap); // By default, blob() returns a new Blob each time (with a different URL, then).
    ...
    overrideInstanceMethod('Audio', 'play', originalPlay => function () {
      if ('' !== this.src.trim() && !getSharedGlobalVariable(KEY_IS_HOWLER_USED)) {
        const audioBlobSoundUrlMap = getSharedGlobalVariable(KEY_AUDIO_BLOB_SOUND_URL_MAP);
        return processSoundPlayback(this, (audioBlobSoundUrlMap === null || audioBlobSoundUrlMap === void 0 ? void 0 : audioBlobSoundUrlMap[this.src]) || this.src, SOUND_PLAYBACK_STRATEGY_AUDIO, () => originalPlay.call(this));
      }
...
export const onSoundPlaybackRequested = _arg31 => {
...
export const onSoundPlaybackCancelled = _arg32 => {
  return registerSoundPlaybackEventListener(EVENT_TYPE_SOUND_PLAYBACK_CANCELLED, _arg32);
};
/**
 * @type {Function}
...
export const onSoundPlaybackConfirmed = _arg33 => {
  return registerSoundPlaybackEventListener(EVENT_TYPE_SOUND_PLAYBACK_CONFIRMED, _arg33);
};
/**
 * @type {string}
 */
...
const registerStorageEventListener = (callback, listenerId = getUniqueEventListenerId()) => {
  ...
  if (!iframe[KEY_HAS_STORAGE_EVENT_LISTENER]) {
    ...
    iframe.contentWindow.addEventListener('storage', event => dispatchEvent(EVENT_TYPE_DUO_STATE_CHANGED, {
      key: event.key,
...
export const onDuoStateChanged = _arg34 => {
  return registerDerivedEventListener(EVENT_TYPE_DUO_STATE_CHANGED, EVENT_TYPE_STORAGE_ITEM_CHANGED, _arg34, ({
    key,
    oldValue: oldState = null,
    newValue: newState = null
  }) => key === 'duo.state' && {
    oldState,
    newState
  }, (_arg2, _arg3, _arg4) => {
    return registerStorageEventListener(_arg3, _arg4);
  });
};