...
export const sendActionRequestToContentScript = async (action, value) => new Promise((resolve, reject) => {
...
export const sendActionRequestToBackgroundScript = async (action, value) => sendMessageToBackgroundScript({
  type: MESSAGE_TYPE_ACTION_REQUEST,
  action,
  value
}).then(result => {
...
export const sendMessageToBackgroundScript = async data => new Promise((resolve, reject) => {
...
export const sendBackgroundEventNotificationToPageScript = (event, value) => {
  ...
  chrome.tabs.query({
    ...
    return _it.forEach(_arg => {
      return chrome.tabs.sendMessage(_arg.id, {
        type: MESSAGE_TYPE_BACKGROUND_EVENT_NOTIFICATION,
        event,
        value
      });
...
export const onBackgroundEvent = (callback, eventTypes) => {
  ...
  const isRelevantEventType = !isArray(eventTypes) ? () => true : _arg2 => {
    return eventTypes.indexOf(_arg2) >= 0;
  };
...
export const setupContentScriptBridge = (actionTypes, uiEventTypes, backgroundEventTypes) => {
  const isRelevantActionType = !isArray(actionTypes) ? () => true : _arg3 => {
    return actionTypes.indexOf(_arg3) >= 0;
